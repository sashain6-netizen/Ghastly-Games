/* =============================================
   ...
   FIXED VERSION — Bug List:
   1.  Player burn/chill/weaken/stun never had mechanical effects — now all implemented
   2.  `atk_up` permanently modified e.atk — now uses a status multiplier, reverts correctly
   3.  `stun` didn't skip the player's turn — now properly suspends player actions for 1 turn
   4.  `weaken` had no damage formula effect — now reduces outgoing damage by 25%
   5.  `chill` had no effect — now doubles player miss chance and reduces dodge
   6.  Player regen only fired in action.multi > 0 branch — now fires in all enemy action branches
   7.  Player burn damage never processed — new processPlayerStatuses() handles it
   8.  `skillOverride` in performPlayerAttack was dead code — cleaned up; skills always go through performPlayerSkill
   9.  Enemy has no low-HP visual — added matching red bar at <25% HP
   10. tickStatuses didn't propagate renders consistently — all paths now call renderStatuses()
   11. `enemy-status` render missing from several code paths — renderStatuses() is now called after every status change
   12. Battle_cry stacked ATK infinitely on retry — fixed with status multiplier approach
   ============================================= */
...
const G = {
  ...
  typingTimer: null,
  battleNumber: 0,
  playerStunned: false,   // FIX #3: track stun state explicitly
};
...
function createPlayer() {
  return {
    ...
    skills: [
      { id: 'ember_slash',   name: 'Ember Slash',   icon: '🔥', desc: 'A blazing sword strike',    cost: 12, type: 'attack', multiplier: 1.8, effect: 'burn',  effectChance: 0.4 },
      { id: 'ash_veil',      name: 'Ash Veil',      icon: '🌑', desc: 'Raise defense this turn',   cost: 8,  type: 'buff',   stat: 'guard', value: 0.4 },
      { id: 'hollow_pierce', name: 'Hollow Pierce', icon: '⚔️', desc: 'Ignores enemy defense',     cost: 18, type: 'pierce', multiplier: 1.5 },
      { id: 'cinder_pulse',  name: 'Cinder Pulse',  icon: '💥', desc: 'Area burst, high crit',     cost: 22, type: 'attack', multiplier: 2.2, critBonus: 0.35 },
      { id: 'flame_ward',    name: 'Flame Ward',    icon: '🛡️', desc: 'Heal 30 HP + regen',        cost: 20, type: 'heal',   value: 30, effect: 'regen' },
      { id: 'soul_drain',    name: 'Soul Drain',    icon: '🌀', desc: 'Drains HP and MP',          cost: 15, type: 'drain',  multiplier: 1.2 },
    ],
  };
}
...
const ENEMY_ROSTER = [
  {
    id: 'void_shade',
    name: 'VOID SHADE', art: '👁️',
    hp: 85, maxHp: 85, atk: 16, def: 8, spd: 12,
    xp: 40, gold: 15, statuses: [],
  ...
  {
    id: 'bone_herald',
    name: 'BONE HERALD', art: '💀',
    hp: 130, maxHp: 130, atk: 22, def: 14, spd: 10,
    xp: 75, gold: 28, statuses: [],
  ...
  {
    id: 'echo_knight',
    name: 'ECHO KNIGHT', art: '⚔️',
    hp: 180, maxHp: 180, atk: 28, def: 20, spd: 14,
    xp: 120, gold: 50, statuses: [],
  ...
  {
    id: 'memory_witch',
    name: 'MEMORY WITCH', art: '🔮',
    hp: 145, maxHp: 145, atk: 30, def: 10, spd: 18,
    xp: 100, gold: 40, statuses: [],
  ...
  {
    id: 'hollow_king',
    name: 'THE HOLLOW KING', art: '👑',
    hp: 320, maxHp: 320, atk: 40, def: 25, spd: 16,
    xp: 999, gold: 200, statuses: [],
...
// ─── ENEMY ACTIONS ───────────────────────────
const ENEMY_ACTIONS = {
  strike:         { name: 'strikes',                      multi: 1.0, type: 'physical' },
  shadow_claw:    { name: 'slashes with shadow claws',    multi: 1.3, type: 'physical', effect: 'weaken',  effectChance: 0.3 },
  flee_attempt:   { name: 'attempts to flee but cannot', multi: 0,   type: 'special',  log: 'sys' },
  death_knell:    { name: 'sounds the death knell',       multi: 1.5, type: 'magical',  effect: 'chill',   effectChance: 0.5 },
  bone_ward:      { name: 'raises a bone ward',           multi: 0,   type: 'buff',     buff: 'guard' },
  heavy_strike:   { name: 'delivers a crushing blow',     multi: 1.6, type: 'physical' },
  shield_slam:    { name: 'slams with the shield',        multi: 1.2, type: 'physical', effect: 'stun',    effectChance: 0.25 },
  battle_cry:     { name: 'lets out a battle cry',        multi: 0,   type: 'buff',     buff: 'atk_up' },
  mind_shatter:   { name: 'shatters your mind',           multi: 1.4, type: 'magical',  effect: 'weaken',  effectChance: 0.6 },
  steal_memory:   { name: 'steals a memory',              multi: 0.8, type: 'drain' },
  hex_strike:     { name: 'hexes and strikes',            multi: 1.5, type: 'magical',  effect: 'burn',    effectChance: 0.4 },
  hollow_strike:  { name: 'strikes with hollow power',   multi: 1.8, type: 'physical' },
  void_crush:     { name: 'crushes with void energy',     multi: 2.0, type: 'magical',  effect: 'weaken',  effectChance: 0.7 },
  soul_harvest:   { name: 'harvests your soul essence',  multi: 1.2, type: 'drain',    effect: 'chill',   effectChance: 0.5 },
  dark_edict:     { name: 'issues a dark edict',          multi: 1.6, type: 'magical',  effect: 'burn',    effectChance: 0.8 },
};
...
// ─── STATUS HELPERS ──────────────────────────
// FIX #2 & #4: Returns effective ATK for a combatant, accounting for buffs/debuffs.
// atk_up is now a status with a multiplier rather than a permanent stat mutation.
function getEffectiveAtk(combatant) {
  let atk = combatant.atk;
  const atkUp = (combatant.statuses || []).find(s => s.id === 'atk_up');
  if (atkUp) atk = Math.floor(atk * (1 + (atkUp.mult || 0.2)));
  // FIX #4: weaken reduces outgoing damage by 25% for both player and enemy
  if ((combatant.statuses || []).some(s => s.id === 'weaken')) atk = Math.floor(atk * 0.75);
  return atk;
}
...
function applyStatus(target, status) {
  ...
  if (existing) {
    ...
    // Preserve multiplier data on refresh
    if (status.mult !== undefined) existing.mult = status.mult;
  } else {
...
// ─── STATUS PROCESSORS ───────────────────────
// FIX #1 & #7: Process enemy statuses (burn). Returns true if enemy dies.
async function processEnemyStatuses() {
...
// FIX #1 & #5: Process player statuses (burn, regen).
// Returns true if the player dies from burn.
async function processPlayerStatuses() {
  const p = G.player;

  // Burn damages player
  for (const status of (p.statuses || [])) {
    if (status.id === 'burn') {
      const burnDmg = rand(4, 9);
      p.hp = Math.max(0, p.hp - burnDmg);
      logEntry(`Kael takes ${burnDmg} burn damage!`, 'damage');
      floatOverPlayer(burnDmg, 'player');
      updateCombatUI();
      if (p.hp <= 0) {
...
function startBattle(enemyId, onWin, onLose) {
  ...
  G.player.statuses = [];
  G.playerStunned = false;   // FIX #3: reset stun flag on new battle
...
function updateCombatUI() {
  ...
  // FIX #9: Enemy gets a red bar at low HP, same as player
  $('enemy-hp-bar').style.background = '';
  $('enemy-hp-bar').style.boxShadow = '';
  if (eHpPct < 25) {
    $('enemy-hp-bar').style.background = 'linear-gradient(90deg, #ff1744, #b71c1c)';
    $('enemy-hp-bar').style.boxShadow = '0 0 12px rgba(255,23,68,0.8)';
  }
  ...
  // Reset HP bar before conditionally overriding so it reverts when HP recovers
  ...
  $('player-stats').innerHTML = `
    <span>ATK <span class="stat-val">${p.atk}</span></span>
    <span>DEF <span class="stat-val">${p.def}</span></span>
    <span>LV  <span class="stat-val">${p.level}</span></span>
  `;
...
function renderActions() {
  // FIX #3: If player is stunned, show a message and auto-advance to enemy turn
  if (G.playerStunned) {
    G.playerStunned = false;
    $('action-menu').classList.remove('hidden');
    $('sub-menu').classList.add('hidden');
    const grid = $('action-grid');
    grid.innerHTML = '';
    logEntry('Kael is stunned and cannot act!', 'damage');
    // Auto-trigger enemy turn after a brief delay
...
// ─── PLAYER ACTIONS ──────────────────────────
// FIX #8: Removed dead `skillOverride` parameter — skills always route through performPlayerSkill.
async function performPlayerAttack() {
  ...
  // FIX #5: Chill doubles the player's miss chance (10% → 20%)
  const isChilled = (p.statuses || []).some(s => s.id === 'chill');
  const missThreshold = isChilled ? 0.10 : 0.05;
  if (Math.random() < missThreshold) {
    logEntry(`Kael attacks but misses!${isChilled ? ' (Chilled!)' : ''}`, 'miss');
  ...
  // FIX #4: Use getEffectiveAtk so weaken reduces damage
  const effectiveAtk = getEffectiveAtk(p);
  let dmg = rand(effectiveAtk - 4, effectiveAtk + 6);
...
async function performPlayerSkill(skill) {
  ...
  if (skill.type === 'attack' || skill.type === 'pierce') {
    ...
    // FIX #4: weaken reduces outgoing skill damage too
    const effectiveAtk = getEffectiveAtk(p);
    let dmg = rand(effectiveAtk - 2, effectiveAtk + 8);
    dmg = Math.floor(dmg * skill.multiplier);
    if (skill.type !== 'pierce') {
      dmg = Math.max(1, Math.floor(dmg - e.def * 0.4));
    }
    ...
    // Buff targets the player, not the enemy
...
async function enemyTurn() {
  ...
  // FIX #1: Process enemy statuses (burn). Returns early if enemy died.
  const enemyDied = await processEnemyStatuses();
  if (enemyDied) return;
  ...
  if (!action) {
    // FIX #6: Always process player statuses regardless of action type
    const playerDied = await processPlayerStatuses();
    if (!playerDied) tickStatuses(p);
    unlockCombat();
    renderActions();
    return;
  }
  ...
  if (action.type === 'buff') {
    if (action.buff === 'guard') {
      ...
      // FIX #2 & #4: Store atk_up as a temporary status with a multiplier (mult: 0.2 = +20%)
      // instead of permanently overwriting e.atk — this correctly reverts when the status expires.
      applyStatus(e, { id: 'atk_up', label: '⬆️ATK', duration: 2, mult: 0.2 });
      logEntry(`${e.name} ${action.name}! Its attack rises!`, 'system');
    }
    updateCombatUI();
    await sleep(500);
    // FIX #6: Process player statuses even on buff/special turns
    const playerDied = await processPlayerStatuses();
    if (!playerDied) tickStatuses(p);
    unlockCombat();
    renderActions();
  ...
  if (action.type === 'special') {
    logEntry(`${e.name} ${action.name}.`, 'system');
    await sleep(400);
    // FIX #6: Process player statuses on special turns too
    const playerDied = await processPlayerStatuses();
    if (!playerDied) tickStatuses(p);
    unlockCombat();
    renderActions();
  ...
  if (action.multi > 0) {
    ...
    // FIX #4: Use getEffectiveAtk so enemy atk_up is applied via status, not permanent mutation.
    // Enemy weaken also reduces their damage through getEffectiveAtk.
    const effectiveAtk = getEffectiveAtk(e);
    let dmg = rand(effectiveAtk - 4, effectiveAtk + 4);
    ...
    if (action.effect && Math.random() < (action.effectChance || 0.3)) {
      ...
      // FIX #3: Set stun flag so renderActions suspends the player's next turn
      if (action.effect === 'stun') {
        G.playerStunned = true;
        logEntry('Kael is stunned! Next action will be skipped.', 'damage');
      }
    }
  ...
  // FIX #6 & #7: Always process player statuses (burn, regen) after every enemy action.
  const playerDied = await processPlayerStatuses();
...
async function handleEnemyDeath() {
  ...
  // Use a named function so reassigning onclick on retry doesn't leave stale closures
...
function runBattleSequence(index) {
  ...
  startStory(battle.storyBefore, () => {
    startBattle(battle.enemy,
      () => { startStory(battle.storyAfter, () => { runBattleSequence(index + 1); }); },
      () => { /* handled inside battle */ }
    );
  });
}
...
function unlockCombat() {
  G.combatLocked = false;
  // Note: disabled buttons are intentionally left until renderActions() recreates them fresh.
}
...
function init() {
  ...
  $('btn-new-game').addEventListener('click', () => {
    ...
    G.battleNumber = 0;
    flash();
    startStory('intro', () => { runBattleSequence(0); });
  });
